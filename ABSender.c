 ...
 */
    ...
    fprintf(stderr, "    -j pathfile         File containing data in request path and in Range Mode\n");
    fprintf(stderr, "    -O cookiefile       File containing data in request cookie\n");
    fprintf(stderr, "    -o cookiefile       File containing data in request cookie and in Range Mode\n");
    fprintf(stderr, "    -L headerfile       File containing data in request header\n");
...
static void set_request()
{
    ...
    if (posting <= 0) {
        snprintf_res = apr_snprintf(request, sizeof(_request),
            ...
            cookie, auth, 
            header_file_s.count ? hdrs_cp : hdrs);
    }
    else {
        snprintf_res = apr_snprintf(request,  sizeof(_request),
            "%s %s HTTP/1.0\r\n"
            ...
            cookie, auth,
            postlen,
            (content_type[0]) ? content_type : "text/plain", (header_file_s.count ? hdrs_cp : hdrs));
    }
...
static file_t read_file(const char *file)
{
    ...
    *(databuff+file_len) = '\0'; 
    ...
    {
        dataline = strstr(databuff + pos_cur, "\n"); 
        if(dataline&&index < file_info.count) 
        { 
            line_size = dataline - databuff - pos_cur; 
            // set file_info.content 
            file_info.content[index] = (char*)malloc(line_size + 1); 
            strncpy(file_info.content[index], databuff + pos_cur, line_size); 
            *(file_info.content[index] + line_size) = '\0'; 
            if(*(file_info.content[index] + line_size -1) == '\r')
                *(file_info.content[index] + line_size -1) = '\0';
            index++;
            pos_cur += line_size + 1; 
        }
    }while(dataline&&index < file_info.count); 
        ...
        switch (c) {
            ...
            case 'j':
                path_file_s = read_file(optarg);
                    if(path_file_s.count == 0)
                        exit(1);
                path_range_s = parseRange(path_file_s);
                is_path_range = 1;
                break;
            case 'O':
                cookie_file_s = read_file(optarg);
                    if(cookie_file_s.count == 0)
                        exit(1);
                break;
            case 'o':
            ...
            case 'l':
                header_file_s = read_file(optarg);
                    if(header_file_s.count == 0)
                        exit(1);
                header_range_s = parseRange(header_file_s);
                is_header_range = 1;
                break;
            case 'Y':
                if (posting != 0)
                    err("Cannot mix POST and HEAD\n");
                multi_post_file_s = read_file(optarg);
                    if(multi_post_file_s.count == 0)
                        exit(1);
                posting = 3;
                break;
            case 'R':
                ...
                multi_post_file_s = read_file(optarg);
                    if(multi_post_file_s.count == 0)
                        exit(1);
                post_range_s = parseRange(multi_post_file_s);
                is_post_range = 1;
                posting = 3;
                break;
            case 'D':
